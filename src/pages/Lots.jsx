...
export default function Lots() {
  ...
  const [detail, setDetail] = useState(null) // venta+cuotas del lote
  ...
  useEffect(() => {
    ...
    async function load() {
      const { data: sale } = await supabase.from('sales')
        .select('*, client:clients!sales_client_id_fkey(full_name, phone, phone_valid, doc_number), advisor:advisors(code, full_name)')
  ...
  const byMz = useMemo(() => {
    const g = {}
    for (const l of lots) {
      if (filter !== 'todos' && l.status !== filter) continue
      ;(g[l.mz] = g[l.mz] || []).push(l)
    }
    for (const k in g) g[k].sort((a, b) => Number(a.lt) - Number(b.lt) || a.lt.localeCompare(b.lt))
    return g
  }, [lots, filter])
  ...
  function waMessage() {
    ...
    if (next) msg += `Próxima cuota: N°${next.installment_number} de S/ ${Number(next.amount).toFixed(2)}, vence el ${next.due_date}.\n`
    msg += `\nPuede pagar por transferencia o depósito. ¡Gracias ${name}! 🙌`
      ...
      {Object.entries(byMz).map(([mz, arr]) => (
        <section key={mz} className="mz-block">
          ...
          <div className="lot-grid">
            {arr.map(l => (
              <button key={l.id} className="lot-cell"
                style={{ background: COLORS[l.status] }}
                title={`Mz ${l.mz} Lt ${l.lt} — ${LBL[l.status]}`}
                onClick={() => setSel(l)}>
                {l.lt}
              </button>
            ))}
      ...
      {sel && (
        <div className="modal-bg" onClick={() => setSel(null)}>
          <div className="glass modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="ficha">
              <p><span className="muted">Área:</span> {sel.area_m2} m²</p>
              <p><span className="muted">Precio/m²:</span> S/ {Number(sel.price_per_m2).toFixed(2)}</p>
              ...
              <>
                ...
                <div className="ficha">
                  ...
                  {detail.sale.client?.phone_valid
                    ...
                    : <p className="error">⚠ Teléfono no válido — actualizar en la ficha del cliente</p>}
                </div>
              </>
