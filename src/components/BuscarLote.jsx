...
// Quien esta ligado a cada lote del proyecto: titular y co-comprador de la venta
// viva, o quien lo separo. Se consulta recien al usar el buscador, no al abrir
// la pantalla: la mayoria de las veces nadie lo toca.
async function cargarGente(pid) {
  const [l, v, s] = await Promise.all([
    supabase.from('lots').select('id, mz, lt, status').eq('project_id', pid).order('mz').order('lt'),
    supabase.from('sales')
...
export default function BuscarLote({ autoFocus, placeholder = 'Buscar lote (G7), nombre, DNI o celular…' }) {
  ...
  useEffect(() => { setDatos(null) }, [pidOp])

  async function asegurarDatos() {
    if (datos || cargando || !pidOp) return
    setCargando(true)
    try { setDatos(await cargarGente(pidOp)) } finally { setCargando(false) }
  }
            ...
            {resultados.map((l, k) => {
              ...
              return (
                <button type="button" key={l.id} className={`bsc-op bl-op ${k === i ? 'foco' : ''}`}
                  ...
                  <span className="bsc-sub muted">{p?.doc_number && !String(p.doc_number).startsWith('PEND') ? p.doc_number + ' · ' : ''}{LBL[l.status]}</span>
                </button>
              )
            })}
